...
def resolve_report_temporal_sidecar(
    *,
    ...
    for source, container in candidates:
        ...
        if compact:
            diagnostics.append(f"{source}: stale Interval-wise Activity-State Concordance contract or config")
    ...
    for path in artifact_paths:
        ...
        try:
            ...
            if not full_dynamic_is_current(full_sidecar):
                diagnostics.append(f"{path}: stale Interval-wise Activity-State Concordance contract or config")
                continue
