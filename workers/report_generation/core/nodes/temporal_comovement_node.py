...
def run_temporal_comovement(state: dict) -> dict:
    ...
    try:
        ...
        # Step 8: Compute divergence once and share its canonical observation-first
        # records with both legacy LLM context and P5 candidate discovery.
        tmm_site_contributions = (state.get("kinase_activity_heatmap") or {}).get("tmm_site_contribution_matrix", {})
        multisite_divergence = _compute_multisite_divergence_for_report(
            sig_matrix, sig_meta, timepoints, ptm_type=ptm_type,
...
def _build_comovement_llm_context(
    ...
    enriched_data: Optional[list] = None,
    tmm_site_contributions: Optional[dict] = None,
    multisite_divergence: Optional[list] = None,
) -> str:
    ...
    # v13.0: narrative now generated by TemporalDivergencePair.to_ai_sentence()
    #        via build_ai_divergence_summary(); statistics metadata still from
    #        _compute_multisite_divergence_for_report() for cross-reference section.
    divergence_pairs = (
        multisite_divergence
        if multisite_divergence is not None
        else _compute_multisite_divergence_for_report(
    ...
    if divergence_pairs:
        ...
        if _first_pair_warning:
            parts.append(f"\n⚠️ {_first_pair_warning}\n")

        # Convert to TemporalDivergencePair objects for AI-quality narrative
        try:
            from common.biological_relationship import (
                TemporalDivergencePair as _TDPReport,
                build_ai_divergence_summary as _build_ai_div,
            )

            def _to_tdp(p: dict, _ptm_type: str) -> "_TDPReport":
                sA = p.get("siteA", {})
                sB = p.get("siteB", {})
                gene = p.get("gene", "")
                _lag = abs(
                    sB.get("peak_tp_idx", 0) - sA.get("peak_tp_idx", 0)
                )
                _fcA = float(sA.get("peak_fc", 0.0))
                _fcB = float(sB.get("peak_fc", 0.0))
                return _TDPReport(
                    protein=gene,
                    siteA=f"{gene} {sA.get('site', '')}",
                    siteB=f"{gene} {sB.get('site', '')}",
                    pattern=p.get("pattern", ""),
                    peak_condA=str(sA.get("peak_tp", "")),
                    peak_condB=str(sB.get("peak_tp", "")),
                    temporal_lag=_lag,
                    fcA=_fcA,
                    fcB=_fcB,
                    fc_ratio=round(abs(_fcB) / max(abs(_fcA), 0.01), 3),
                    is_denovoA=bool(sA.get("is_de_novo", False)),
                    is_denovoB=bool(sB.get("is_de_novo", False)),
                    ptm_type=_ptm_type,
                )

            _bio_pairs = [_to_tdp(p, ptm_type) for p in divergence_pairs]
            parts.append(_build_ai_div(_bio_pairs, max_pairs=15))
        except Exception as _bio_err:
            logger.warning(f"BiologicalRelationship narrative failed, falling back: {_bio_err}")
            # Fallback: use existing description strings
            _pat_labels_fb = {
            ...
            for _pat in ["signal_attenuation", "sequential_regulation", "multisite_coordination"]:
                _pp = [p for p in divergence_pairs if p["pattern"] == _pat]
                if not _pp:
                    continue
                parts.append(f"### {_pat_labels_fb[_pat]}")
                for pair in _pp[:8]:
                    parts.append(f"- {pair.get('description', '')}")
                parts.append("")
