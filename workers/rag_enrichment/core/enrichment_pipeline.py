...
class RAGEnrichmentPipeline:
    ...
    def enrich_ptm_data(
        ...
        """
        Enrich a list of PTM entries with PubMed literature and regulation data.
    ...
    def _enrich_single_ptm(
        ...
        kegg_pathways = [p["name"] for p in kegg_info.get("pathways", [])]

        # 4. STRING-DB interactions via MCP
        string_info = self.mcp.query_stringdb(gene)
        interactions = string_info.get("interactions", [])
        interaction_partners = [f"{i['partner']}({i['score']})" for i in interactions[:5]]
        ...
        classification = self._classify_ptm(ptm_log2fc, protein_log2fc)

        # 8. Build enrichment result
        enrichment = {
            "search_summary": {
            ...
            "string_interactions": interaction_partners,
    ...
    def _classify_ptm(self, ptm_log2fc: float, protein_log2fc: float) -> dict:
        try:
            ...
            return {"quadrant": "unknown", "interpretation": "Insufficient data"}

        if ptm_fc > 0 and prot_fc > 0:
            return {"quadrant": "Q1", "interpretation": "Both protein and PTM upregulated — amplified signaling"}
        elif ptm_fc > 0 and prot_fc < 0:
            return {"quadrant": "Q2", "interpretation": "Protein down but PTM up — active PTM regulation despite protein decrease"}
        elif ptm_fc < 0 and prot_fc < 0:
            return {"quadrant": "Q3", "interpretation": "Both down — reduced signaling"}
        elif ptm_fc < 0 and prot_fc > 0:
            return {"quadrant": "Q4", "interpretation": "Protein up but PTM down — possible dephosphorylation/deubiquitination"}
        else:
            return {"quadrant": "center", "interpretation": "Minimal change"}
