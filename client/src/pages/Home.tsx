      ...
      <header className="sticky top-0 z-40 bg-background/80 backdrop-blur-md border-b border-border">
        <div className="container flex items-center h-14 gap-4">
          ...
          <span className="text-xs text-muted-foreground font-mono hidden sm:block">
            Pipeline Manual v1.8
          </span>
          <div className="ml-auto flex items-center gap-3">
            <span className="text-xs text-muted-foreground">2026-04-06</span>
          </div>
        </div>
      </header>
    ...
    "report_title": "PTM Comprehensive Analysis Report",
          ...
          <DataTable
            ...
            rows={[
              ...
              ["2. 유의 필터링", "낮은 분산/진폭의 PTM 제거 (flat lines). v9.36: Minor PTM 포함을 위해 임계값 완화", "variance ≥ 0.3, amplitude ≥ 0.8"],
          ...
          <DataTable
            ...
            rows={[
              ...
              [
                ...
                "PTM Co-movement Heatmap — 클러스터별 시계열 Log2FC 히트맵. Activity class sidebar: orange=De novo, blue=Regulated, green=Minor",
              ],
              [
                <strong>Figure N+1</strong>,
                "Line Plot",
                <code className="text-xs font-mono">temporal_comovement</code>,
                "Cluster Line Plot — 각 클러스터의 평균 시계열 프로파일. Activity class: orange ★=De novo, blue ●=Regulated, green ◆=Minor",
              ],
              [
          ...
          <Callout type="warning">
            <strong>v9.36: Minor PTM 시각화 개선.</strong> 이전 버전에서 Minor PTM은 회색 점선(반투명 0.35)으로 표시되어 시각적으로 무시되는 문제가 있었습니다.
            v9.36에서는 다음과 같이 변경됩니다:
            (1) <strong>Vector Plot</strong>: Minor PTM 라인이 녹색 실선(opacity 0.75, lineWidth 1.8)으로 변경,
            (2) <strong>Co-wave Heatmap/Line Plot</strong>: Minor 색상 팔레트가 회색 → 녹색으로 변경, 실선(alpha 0.65, lineWidth 1.2),
            (3) <strong>Co-wave 필터링</strong>: MIN_AMPLITUDE 1.5 → 0.8, MIN_VARIANCE 0.5 → 0.3으로 완화하여 패턴이 있는 Minor PTM도 클러스터링에 포함,
            (4) <strong>Signal Flow/Kinase Module</strong>: Minor PTM chip이 녹색 테마(◇ 아이콘, green border)로 변경.
          </Callout>
  ...
  "rag_collection_ids": [1, 3]
          ...
          <div className="mt-12 sm:mt-20 pt-6 sm:pt-8 border-t border-border">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-1 text-[10px] sm:text-xs text-muted-foreground">
              <span>PTM Platform Pipeline Manual v1.8</span>
              <span>Generated by Manus AI &middot; 2026-04-06</span>
            </div>
          </div>
        </main>
      </div>
