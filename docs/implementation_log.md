...
### [2026-09-14] Reader Report 흐름 구멍 정정 (펜스 JSON·문헌·질문·캐시)

- **분류:** 정정
- **대상:** `docs/official_temporal_terminology_contract.md`,
  `workers/common/model_json.py`,
  ...
  `workers/report_generation/core/reader_authoring.py`,
  `workers/report_generation/core/figure_manifest.py`,
  `ptm_shared/vector_plot.py`, `api-server/app/api/orders.py`
- **구현 대상 설계:** `docs/official_temporal_terminology_contract.md`
  (2026-09-14 재선언). 측정 공식 변경 아님.
- **사전등록 상태:** 결과 열람 전 표시·검색·캐시 계약. primary 승격 금지.
- **내용:** 구조화 본문/문헌 비교 JSON은 펜스·여분 문장을 허용한다.
  선정 finding ID는 discovery category로 덮여도 검색한다.
  agreement/disagreement/site 비교는 quote 부분 문자열만 허용하고
  배경 관계만 패러프레이즈를 허용한다. kinase 질문을 단백질 보정
  질문으로 바꾸지 않는다. 문헌 20편·섹션 최소 단어는 제품 목표이며,
  확보 문헌이 20편 미만이거나 본문이 짧기만 한 경우는 draft 사유가
  아니다. Main heatmap 범위는 선택과 동일하게 12–16이다. 색 문구는
  protein-adjusted contrast다. TMM heatmap 캐시는 vector TSV hash를
  포함한다. 원장과 표시 계약을 git 추적 대상으로 복구했다.
- **논문에서의 용도:** methods (reader Report 표시·검색 계약) / 사용 안 함
  (kinase 측정)
- **해석 한계:** 실 Gemini/RAG 성공률, 문헌 의미 검증, locked score를
  바꾸거나 보장하지 않는다.
- **결정성:** heatmap 행 = 12–16. 문헌 제품 목표 = 확보 문헌 ≥ 20일 때
