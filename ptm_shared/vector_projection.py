...
from ptm_shared.quantitative_fields import project_axis_fields, QUANTITATIVE_SCHEMA_VERSION


from ptm_shared.feature_identity import canonical_feature_identity

REPORT_VECTOR_PROJECTION_VERSION = "report_vector_projection.v2"
...
def project_report_vector_row(row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    modified sequence plus charge. Rows without either remain available to diagnostic
    ...
    projected = {
        ...
        "normalization_policy": _text(row, "Normalization_Policy", "normalization_policy"),
        "biological_unit_counts": {
            axis: {arm: _optional_float(row, f"{axis}_{arm}_Biological_N") for arm in ("Control", "Treatment")}
            for axis in ("PTM_Unadjusted", "PTM_ProteinAdjusted")
        },
    ...
    projected.update(canonical_feature_identity(row))
